...
from common import tools, project_paths, generate
...
def main(argv=None):  # pylint: disable=unused-argument
    ...
    train_data = tools.extract_data(train_data_filename, TRAINING_SIZE, IMG_PATCH_SIZE)
    train_labels = tools.extract_labels(train_labels_filename, TRAINING_SIZE, IMG_PATCH_SIZE)
    ...
    def get_prediction(img):
        data = np.asarray(tools.img_crop(img, IMG_PATCH_SIZE, IMG_PATCH_SIZE))
        data_node = tf.constant(data)
        output = tf.nn.softmax(model(data_node))
        output_prediction = s.run(output)
        img_prediction = tools.label_to_img(img.shape[0], img.shape[1], IMG_PATCH_SIZE, IMG_PATCH_SIZE, output_prediction)
    ...
    with tf.Session() as s:
            ...
            for iepoch in range(num_epochs):
                ...
                for step in range (int(train_size / BATCH_SIZE)):
                    ...
                    if step % RECORDING_STEP == 0:
                        ...
                        print ('Minibatch error: %.1f%%' % tools.error_rate(predictions,
                                                                     batch_labels))
        ...
        #for f in os.listdir(test_dir):
            #print("Predicting " + f + "...")
            #img = mpimg.imread(test_dir + f)
            #img_pred = get_prediction(img)
            #img_prediction = solution_to_img(img_pred)

            #Image.fromarray(img_prediction).save(mask_test_dir + f)

            # return (img, img_prediction)
            #pimg = concatenate_images(img, img_pred)
            #Image.fromarray(pimg).save(prediction_test_dir + "concat_" + f)

            #oimg = make_img_overlay(img, img_pred)
            #oimg.save(prediction_test_dir + "overlay_" + f)
